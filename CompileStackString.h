...
namespace StackString {
	...
	template <typename T>
	struct ByteIO<T, 1> {
		_STACKSTRING_FORCE_INLINE static constexpr void to(T Value, unsigned char(&out)[1]) noexcept {
			out[0] = static_cast<unsigned char>(Value);
		}

		_STACKSTRING_FORCE_INLINE static constexpr T from(const unsigned char(&in)[1]) noexcept {
	...
	template <typename T>
	struct ByteIO<T, 2> {
		_STACKSTRING_FORCE_INLINE static constexpr void to(T Value, unsigned char(&out)[2]) noexcept {
		...
		_STACKSTRING_FORCE_INLINE static constexpr T from(const unsigned char(&in)[2]) noexcept {
	...
	template <typename T>
	struct ByteIO<T, 4> {
		_STACKSTRING_FORCE_INLINE static constexpr void to(T Value, unsigned char(&out)[4]) noexcept {
		...
		_STACKSTRING_FORCE_INLINE static constexpr T from(const unsigned char(&in)[4]) noexcept {
	...
	public:
		...
		_STACKSTRING_FORCE_INLINE constexpr StackString(T* pData) noexcept {
			...
			for (std::size_t i = 0; i < kLength; ++i) {
				...
				for (std::size_t k = 0; k < sizeof(T); ++k) {
					const std::size_t j = i * sizeof(T) + k;
					m_Storage[j] = static_cast<unsigned char>(bytes[k] ^ 0xFF);
				}
			}
		}
...
#define _STACKSTRING(STRING)                                                                                                                                                                                   \
	([]() -> auto {                                                                                                                                                                                            \
		constexpr size_t unLength = std::extent_v<std::remove_reference_t<decltype(STRING)>>;                                                                                                                  \
		constexpr auto Encrypted = StackString::StackString<unLength, StackString::clean_type<decltype(STRING[0])>, __LINE__, __COUNTER__>(const_cast<StackString::clean_type<decltype(STRING[0])>*>(STRING)); \
		return Encrypted.Decrypt();                                                                                                                                                                            \
	} ())

#define STACKSTRING(STRING) _STACKSTRING(STRING)
